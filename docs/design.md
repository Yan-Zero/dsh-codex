...
The account usage projection also preserves the backend-owned `rate_limit_upsell` model instructions used by the official Codex client. If the current model is explicitly listed as blocked, the adapter selects the first fallback that exists in the active catalog and keeps the requested reasoning effort only when the replacement supports it. A quota failure can trigger one forced usage refresh and one retry on that server-authorized replacement; absent or malformed upsell data leaves the original request behavior unchanged. This allows eligible accounts to reach a backend-provided Luna Reserve fallback without inventing a client-side Reserve flag or using a first-party originator.
