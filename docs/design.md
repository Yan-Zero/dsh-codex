...
`dsh-codex` is a standard DeepSeek Harness bundle. It adds ChatGPT OAuth, the Codex model catalog, settings-owned custom context, a Codex standalone-search provider, browser account settings, an optional `read_image` URL extension, and `imagegen` without modifying dsh source code. The active dsh profile continues to own the agent loop, attachments, filesystem policy, tools, permissions, compaction, and Web composer.
...
Codex app-server exposes experimental `additionalContext`, but its downstream Responses request contains ordinary input messages rather than an `additional_context` field. This plugin follows that wire behavior: the Settings document stores one bounded fragment and its `application` or `untrusted` trust level, and the response runtime prefixes a standard developer or user message with the fixed `dsh_custom_context` source wrapper. Matching the upstream renderer, fragment values remain unescaped and the Responses role is the authority boundary; the wrapper is only a provenance hint. Empty text adds no item. The stable prefix remains part of pi-ai's full input comparison, so unchanged context permits WebSocket delta continuation while any settings change forces the next request onto full input. Native compaction output does not retain the fragment; every later ordinary request derives it again from the live Settings document.
