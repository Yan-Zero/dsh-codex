...
The account usage projection preserves only the two backend-owned `rate_limit_upsell` recovery forms understood by the current Codex client: ordered `model_recovery`, and `luna_reserve`. Automatic recovery is disabled by default. When enabled, a cached successful usage read may choose the fallback before a healthy request without delaying it; a `QUOTA` attempt forces one usage refresh and owns one retry only when that refresh resolves a different compatible model. Otherwise the original failure remains available to the ordinary Harness retry waterfall.

Recovery runs in the Agent `agent/request` and `agent/request-error` waterfalls, not inside the provider stream. DSH therefore prepares and records the actual fallback route before dispatch. Ordinary candidates must resolve through the active provider catalog. Luna Reserve is the backend-authorized hidden `gpt-reserve` route and borrows the normal Luna descriptor, matching Codex's use of Luna metadata while keeping Reserve out of manual model discovery. Unsupported reasoning effort falls back to the replacement's default. A vision-capable source route skips candidates that do not explicitly accept image input, even before the current request contains an image, so its attachment and image-tool surface does not disappear after recovery. Fast Mode is omitted from Reserve requests; context-window overrides do not replace Reserve's borrowed capacity.
...
`imagegen` executes against the GPT Image route selected in live settings, independently of the current conversation model. The allowlist contains the official Codex default `gpt-image-2` plus the tested subscription routes `gpt-image-2.5`, `gpt-image-2.5-sunburst`, and `gpt-image-2.5-flare`; the default remains `gpt-image-2`. The caller must still declare image input because the tool result contains an image block used by the next model turn. A generation has no references; an edit accepts up to five workspace paths or the most recent one to five conversation image attachments. These two selectors are mutually exclusive. Path reads use `ctx.fs`; conversation references use the attachment store. Base64 data URLs exist only in the private provider request.

The hidden Luna Reserve descriptor retains Luna's image modality and request projection, so attachment admission and `read_image` continue to use the actual prepared route. Automatic fallback never changes the configured image-generation route, including when the conversation request is served by `gpt-reserve`.
...
Live settings persist the image model and two independent switches. `modifyReadImage` adds or removes the scoped `read_image` definition for every live agent; disabling it immediately restores the original Harness definition and schema. `shareImagegenWithOtherModels` controls whether another provider's vision model may execute `imagegen`; Codex vision models retain access. Both switches default to enabled.
