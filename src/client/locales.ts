...
export const en = {
  ...
  contextWindowDefaults: 'Current provider defaults: {values}.',
  contextWindowHint: 'Leave blank to use each model’s provider default. One override applies to every Codex model; an open conversation’s meter refreshes after its next request. This does not increase the backend’s actual capacity, so an unsupported value can still overflow.',
...
export const zh: { [Key in OpenAICodexSettingsKey]: string } = {
  ...
  contextWindowDefaults: '当前提供方默认值：{values}。',
  contextWindowHint: '留空会使用各模型的提供方默认值；一个覆盖值会应用到所有 Codex 模型，已打开会话的用量显示会在下一次请求后刷新。该设置不会提高后端的真实容量，超过模型能力时请求仍可能溢出。',
